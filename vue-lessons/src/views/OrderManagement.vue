    ...
    export default {
        ...
        methods: {
            ...
            if (this.searchId) {
                filteredItems = filteredItems.filter((item) =>
                    item.or_number.includes(this.searchId)
                );
            }
            ...
            this.totalPage = totalPages; // 將 totalPage 設為新計算的值
            },
            setPage(page) {
        ...
        watch: {
            selectedState: {
                ...
                handler() {
                    this.currentPage = 1;
                    this.calculateTotalPages(); // 在這裡重新計算 totalPage
                },
            },
            searchId: {
                immediate: true,
        ...
        computed: {
            filtered_list() {
                ...
                let filteredItems = this.items.filter((item) => {
                    ...
                    return false; // 如果没有匹配的状态，则返回 false
                }).filter((item) => {
                    if (this.current.length === 0) {
                    return true;
                    } else {
                    return this.current.includes(item.or_class);
                    }
                }).filter((item) => {
                    if (this.searchId === '') {
                    return true;
                    } else {
                    return item.or_number.includes(this.searchId);
                    }
                });
                ...
                let sortedItems = filteredItems.map((item) => ({
                    ...
                    return this.sortOrder === 'asc' ? a.or_order_date - b.or_order_date : b.or_order_date - a.or_order_date;
                }).map((item) => ({
                    ...item,
